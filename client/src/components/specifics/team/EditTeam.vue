...
export default {
    ...
    data() {
        return {
            ...
            name: TeamStore(state => state.currentTeam.name),
            disabled: false
        }
    },
    ...
    methods: {
        ...
        addTeam(event) {
            ...
            if (this.name.trim().length > 0) {
                ...
                teamService.update(this.team.id, {name: this.name}).then(async (response) => {
                    const jsonResponse = await response.json();
                    if (response.ok) {
                        TeamStore(state => state.update(this.team.id, jsonResponse.data));
                    }
                }).finally(() => {
                    this.disabled = false;
